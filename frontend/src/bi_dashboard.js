...
const BI = (() => {
  ...
  // ── Sub-tabs: Operación / Análisis ────────────────────────────────────────
  function switchView(view) {
    const opView = document.getElementById('bi-view-operacion');
    const anView = document.getElementById('bi-view-analisis');
    if (!opView || !anView) return;

    opView.hidden = view !== 'operacion';
    anView.hidden = view !== 'analisis';

    document.querySelectorAll('.bi-subtab').forEach(btn => {
      btn.classList.toggle('bi-subtab--active', btn.dataset.view === view);
    });

    if (view === 'operacion') {
      if (typeof BIOp !== 'undefined') BIOp.init();
    } else {
      if (!_state.initialized) init();
      else _fetchAndRender();
    }
  }
  ...
  return {
    ...
    refresh() {
      const activeView = document.querySelector('.bi-subtab--active')?.dataset.view || 'operacion';
      if (activeView === 'analisis') {
        _state.initialized = false;
        _state.rawData     = [];
        _fetchAndRender();
      } else if (typeof BIOp !== 'undefined') {
        BIOp.refresh();
      }
    },
  };
