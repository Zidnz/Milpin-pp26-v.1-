...
function actualizarPanelDetalle(p, esDesdeAPI) {
    ...
    <div style="font-family:'Segoe UI',sans-serif;">
        ...
        ${consumoHTML}
        ${p.id_parcela ? `
        <button class="map-btn-sim3d"
                onclick='window.SIM3D && SIM3D.abrir("${p.id_parcela}", ${JSON.stringify({
                    nombre: p.nombre, cultivo: p.cultivo,
                    area_ha: p.area_ha, sistema_riego: p.sistema_riego,
                }).replace(/'/g, "&#39;")})'>
            ...
            Simular ciclo en 3D · Tradicional vs MILPÍN
        </button>` : ''}
    </div>`;
